            ...
            <div className="mx-10 max-h-24 overflow-y-scroll">
                {comments.map((comment, id)=>(
                    <div key={id} className="flex items-center space-x-2 mb-2 justify-between">
                        <img src={comment.data().userImage} alt="user image" className="h-7 rounded-full object-cover border p-[2px]" />
        ...
        )}
        {
            session && (
                <form onSubmit={handleSubmit} className="flex items-center p-4 gap-2">
                    <img src={session.user.image} alt="user image" className="h-10 w-10 rounded-full border p-[4px] object-cover"  />
